...
if (isset($_POST) && !empty($_POST)) {
    ...
    if (isset($_POST['orderNo']) && !empty($_POST['orderNo'])) {
        ...
        if (isset($_POST['txnRefNo']) && !empty($_POST['txnRefNo'])) {
            $Sambhavpay->setTxnRefNo($_POST['txnRefNo']);
        }else {
    ...
    if (empty($error)) {
        $resStatus = $Sambhavpay->getTrnStatus();
...
function statusRespHandler($jsonData){
    ...
    $mid = "900000000000008"; // Provided by SambhavPay
    $secretKey = "scrh0e0TZiA6J6bKXvJs5Pme8CMavx0cNmi"; // Provided by SambhavPay
    $saltKey = "sal9XIXl94aP3ZC6ZFIki32ugGXBVJBfr"; // Provided by SambhavPay
    ...
    $response = $Sambhavpay->parseStatusResp($respData,$checkSum,$mid);
